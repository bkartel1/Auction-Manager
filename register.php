  ...
  if (count($errs) == 0) {
    ...
    if(mysqli_num_rows($q) == 0) {
	  ...
	  $d = mysqli_query($connection, "UPDATE $Adb SET buyer_id=IF('".$userBuyerID."'='',NULL,'".$userBuyerID."'), seller_id=IF
              ('".$userSellerID."'='',NULL,'".$userSellerID."'), email=IF('".$userEmail."'='',NULL,'".$userEmail."'), street='".$userStreet."', city='".$userCity."', state='".$userState."', zip='".$userZip."', id='".$userID."', phone='".$userPhone."', birth='".$userBirth."', tax='".$userTax."', taxid='".$userTaxID."', buyer_commission='".$userBuyercomm."', seller_commission='".$userSellercomm."', notes='".$userNotes."', setup='".$userSetup."', disposal='".$userDisposal."', disposal_note='".$userDisNotes."' WHERE first_name='".$userFirstName."' AND last_name='".$userLastName."' AND birth='".$userBirth."'");
...
<!DOCTYPE html>
	...
	<div id="userInfo" <?php if ($action == user) echo "style=\"display: block;\""; ?>>
      ...
      <p>The list below is customers from previous auctions.</p>
      <p>Click on a name to register for this auction</p>
      <input type="text" id="myuserInput" onkeyup="mynewFunction()" placeholder="Search for names..">
      <ul id="myULA">
<?php

//var firstName, lastName, userBuyerID, userSellerID, email;
        $userQuery = mysqli_query($connection, "SELECT * from masterusers ORDER BY last_name, first_name");
        while ($result = mysqli_fetch_array($userQuery)) {
          ...
          echo "        ";
          echo "<li><a href=\"#Register\" onclick=\"" . $js . ";\">";
